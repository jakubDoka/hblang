...
pub fn build(self: *Codegen, func: *Types.Func) !void {
    const param_count, const return_count, const ret_abi = func.computeAbiSize(self.abi, self);
    ...
    for (self.func.args, ast.exprs.view(ast.exprs.get(self.func.key.ast).Fn.args)) |ty, aarg| {
        const abi = self.abi.categorize(ty, self);
        abi.types(params[i..]);
        ...
        const arg = switch (abi) {
            ...
            .ByValuePair => |p| b: {
                const slot = self.bl.addLocal(ty.size(self));
    ...
    _ = self.emit(.{ .scope = func.key.scope }, ast.exprs.get(self.func.key.ast).Fn.body);
...
pub fn ensureLoaded(self: *Codegen, value: *Value) void {
    if (value.id == .Ptr) {
        value.id = .{ .Value = self.bl.addLoad(value.id.Ptr, self.abi.categorize(value.ty, self).ByValue) };
    }
}
...
pub fn typeCheck(self: *Codegen, ctx: Ctx, expr: Ast.Id, got: *Value, expected: Types.Id) bool {
    ...
    if (!got.ty.canUpcast(expected, self)) {
    ...
    if (got.ty != expected) {
        if (got.ty.isSigned() and got.ty.size(self) < expected.size(self)) {
            got.id.Value = self.bl.addUnOp(.sext, self.abi.categorize(expected, self).ByValue, got.id.Value);
        }

        if (got.ty.isUnsigned() and got.ty.size(self) < expected.size(self)) {
            //self.report(expr, "{} {} {}", .{ got.ty, got.id.Value.data_type, expected });
            got.id.Value = self.bl.addUnOp(.uext, self.abi.categorize(expected, self).ByValue, got.id.Value);
        }
...
fn emitStructOp(self: *Codegen, ty: *Types.Struct, op: Lexer.Lexeme, loc: *Node, lhs: *Node, rhs: *Node) void {
    var offset: usize = 0;
    for (ty.getFields(self)) |field| {
        offset = std.mem.alignForward(usize, offset, field.ty.alignment(self));
        ...
        if (field.ty.data() == .Struct) {
            ...
            const dt = self.abi.categorize(field.ty, self).ByValue;
...
pub fn emitGenericStore(self: *Codegen, loc: *Node, value: *Value) void {
    ...
    if (self.abi.categorize(value.ty, self) == .ByValue) {
        self.ensureLoaded(value);
        _ = self.bl.addStore(loc, self.abi.categorize(value.ty, self).ByValue, value.id.Value);
    } else if (value.id.Ptr != loc) {
        _ = self.bl.addFixedMemCpy(loc, value.id.Ptr, value.ty.size(self));
    }
}

pub fn resolveTy(self: *Codegen, ctx: Ctx, expr: Ast.Id) Types.Id {
    const value = self.emitTyped(ctx, .type, expr);
    return self.unwrapTyConst(value.id.Value);
}
...
pub fn emit(self: *Codegen, ctx: Ctx, expr: Ast.Id) Value {
    ...
    switch (ast.exprs.get(expr)) {
        ...
        .Integer => |e| {
            ...
            return mkv(ty, self.bl.addIntImm(self.abi.categorize(ty, self).ByValue, parsed));
        },
        .Bool => |e| {
        ...
        .Ident => |e| for (self.scope.items, 0..) |se, i| {
            ...
            const typ = if (ty) |typ| b: {
                const global = self.types.resolveGlobal(cursor.scope, ast.tokenSrc(e.id.pos()), value);
                @import("Comptime.zig").evalGlobal(self.types, global.data().Global, typ, value);
                break :b global;
            } else return self.emitTyConst(self.resolveTy(cursor.addName(ast.tokenSrc(e.id.pos())), value));
        ...
        .Ctor => |e| {
            ...
            const local = ctx.loc orelse self.bl.addLocal(ty.size(self));
            ...
            for (ast.exprs.view(e.fields)) |f| {
                ...
                var offset: usize = 0;
                const ftype = for (struct_ty.getFields(self)) |tf| {
                    offset = std.mem.alignForward(usize, offset, tf.ty.alignment(self));
                    if (std.mem.eql(u8, tf.name, fname)) break tf.ty;
                    offset += tf.ty.size(self);
                } else {
        ...
        .Tupl => |e| {
            ...
            var offset: usize = 0;
            for (ast.exprs.view(e.fields), struct_ty.getFields(self)) |field, sf| {
                const ftype = sf.ty;
                offset = std.mem.alignForward(usize, offset, ftype.alignment(self));

                const off = self.bl.addFieldOffset(local, @intCast(offset));
                var value = self.emit(ctx.addTy(ftype).addLoc(off), field);
                if (self.typeCheck(ctx, field, &value, ftype)) continue;
                self.emitGenericStore(off, &value);
                offset += ftype.size(self);
            }
        ...
        .UnOp => |e| switch (e.op) {
            ...
            .@"-" => {
                ...
                return mkv(lhs.ty, self.bl.addUnOp(.neg, self.abi.categorize(lhs.ty, self).ByValue, lhs.id.Value));
            },
        ...
        .BinOp => |e| switch (e.op) {
            inline .@":=", .@":" => |t| {
                ...
                self.bl.resizeLocal(loc, value.ty.size(self));
            ...
            .@"=" => if (e.lhs.tag() == .Wildcard) {
                ...
                const unified: Types.Id = ctx.ty orelse lhs.ty.binOpUpcast(rhs.ty, self) catch |err| {
                ...
                if (lhs.ty.data() == .Struct) if (e.op.isComparison()) {
                    ...
                    const loc = ctx.loc orelse self.bl.addLocal(unified.size(self));
                    ...
                    return mkv(unified, self.bl.addBinOp(
                        e.op.toBinOp(lhs.ty),
                        self.abi.categorize(unified, self).ByValue,
                        lhs.id.Value,
                        rhs.id.Value,
                    ));
                }
        ...
        .Call => |e| {
            ...
            if (ret_abi == .ByRef) {
                ...
                args.arg_slots[i] = ctx.loc orelse self.bl.addLocal(func.ret.size(self));
                i += 1;
            } else {
            ...
            if (caller) |*value| {
                const abi = self.abi.categorize(value.ty, self);
            ...
            return switch (ret_abi) {
                ...
                .ByValuePair => |pair| b: {
                    const slot = ctx.loc orelse self.bl.addLocal(func.ret.size(self));
        ...
        .Return => |e| {
            var value = self.emitTyped(ctx.addLoc(self.struct_ret_ptr), self.func.ret, e.value);
            if (self.typeCheck(ctx, e.value, &value, self.func.ret)) return .never;
            switch (self.abi.categorize(value.ty, self)) {
        ...
        .Struct => |e| return self.emitTyConst(self.types.resolveStruct(ctx.scope, ctx.file(), ctx.name, expr, e.fields)),
        .Fn => |e| {
