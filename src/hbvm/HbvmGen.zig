    ...
    for (node.outputs()) |no| {
        ...
        switch (no.kind) {
            ...
            .GlobalAddr => {
                ...
                const slot = try utils.ensureSlot(&self.globals, self.gpa, extra.id);
                try self.out.addReloc(self.gpa, slot, 4, 3);
            ...
            .Call => {
                ...
                if (extra.id == eca) {
                    ...
                    const slot = try utils.ensureSlot(&self.funcs, self.gpa, extra.id);
                    try self.out.addReloc(self.gpa, slot, 4, 3);
