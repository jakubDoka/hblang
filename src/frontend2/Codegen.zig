...
const Ast = root.frontend2.Ast;
const Arena = utils.Arena;
const Comptime = root.frontend2.Comptime;
const Lexer = root.frontend2.Lexer;
const Types = root.frontend2.Types;
const HbvmGen = root.hbvm.HbvmGen;
const Vm = root.hbvm.Vm;
const TySlice = root.frontend2.types.Slice;
const tys = root.frontend2.types;
    ...
    pub fn findCapture(self: Scope, ast: Ast.Pos, id: Ast.Ident, types: *Types, slot: *Types.Scope.Param) ?*const Types.Scope.Param {
        return switch (self) {
            ...
            .Tmp => |t| for (t.scope.items, 0..) |se, i| {
                if (se.name == id) {
                    var value = Value.mkp(se.ty, t.scope_pins.getValue(i));
                    slot.* = .{
                        .ty = se.ty,
                        .id = .{ .has_value = se.@"comptime", .index = @intCast(id.pos()) },
                        .value = if (se.@"comptime") t.partialEvalCapture(ast, &value) catch 0 else 0,
                    };
                    return slot;
                }
            } else null,
...
pub fn build(self: *Codegen, func_id: utils.EntId(tys.Func)) BuildError!void {
    ...
    if (self.abi.isByRefRet(ret_abi)) {
        self.struct_ret_ptr = self.bl.addParam(.none, i, @intFromEnum(func.ret));
        i += 1;
    } else {
        self.struct_ret_ptr = null;
    }
    ...
    for (ast.exprs.view(fn_ast.args)) |a| {
        ...
        const arg = switch (abi) {
            ...
            .ByRef => self.bl.addParam(arg_sloc, i, @intFromEnum(ty)),
            .ByValue => if (params[i] == .Stack)
                self.bl.addParam(arg_sloc, i, @intFromEnum(ty))
            else
            ...
            .Imaginary => b: {
                len = 0;
                break :b self.bl.addLocal(self.src(aarg), 0, @intFromEnum(ty));
            },
        };
        self.scope.appendAssumeCapacity(.{ .ty = ty, .name = ident.id, .@"comptime" = ident.pos.flag.@"comptime" });
        self.scope_pins.push(&self.bl, arg);
        i += len;
        ty_idx += 1;
    }
...
pub fn emitNull(self: *Codegen, ctx: Ctx, expr: Ast.Id) EmitError!Value {
    ...
    if (nullable.nieche.offset(self.types)) |spec| {
        ...
        switch (abi) {
            ...
            .ByValuePair, .ByRef => {
                const loc = ctx.loc orelse self.bl.addLocal(sloc, abi.size(), @intFromEnum(ty));
...
pub fn emitIdk(self: *Codegen, ctx: Ctx, expr: Ast.Id) EmitError!Value {
    ...
    return switch (abi) {
        ...
        .ByValuePair, .ByRef => {
            const loc = self.bl.addLocal(sloc, abi.size(), @intFromEnum(ty));
            return .mkp(ty, loc);
        },
    };
}
...
pub fn emitCtor(self: *Codegen, ctx: Ctx, expr: Ast.Id, e: *Expr(.Ctor)) EmitError!Value {
    ...
    var ty = oty;
    const local = ctx.loc orelse self.bl.addLocal(self.src(expr), self.abiCata(ty).size(), @intFromEnum(ty));
    ...
    switch (ty.data()) {
        ...
        .Union => |union_ty| {
            ...
            const tag = union_ty.getTag(self.types);
            if (tag != .void) {
                const offset = union_ty.tagOffset(self.types);
                const val = tag.data().Enum.getFields(self.types)[i].value;
                const vl_ty = self.abiCata(tag).ByValue;
                const value_cnst = self.bl.addIntImm(sloc, vl_ty, val);
                self.bl.addFieldStore(sloc, local, @intCast(offset_cursor + offset), vl_ty, value_cnst);
            }
        },
        else => {
...
pub fn emitTuple(self: *Codegen, ctx: Ctx, expr: Ast.Id, e: *Expr(.Tupl)) EmitError!Value {
    ...
    if (e.ty.tag() == .Void and ctx.ty == null) {
        const local = ctx.loc orelse self.bl.addLocal(self.src(expr), 0, @intFromEnum(e.ty));
        var offset: u64 = 0;
        var alignment: u64 = 1;
        ...
        const tuple = self.types.makeTuple(types);
        if (ctx.loc == null) self.bl.resizeLocal(local, self.abiCata(tuple).size(), @intFromEnum(tuple));

        return .mkp(tuple, local);
    } else {
        const oty = ctx.ty orelse try self.resolveAnonTy(e.ty);
        var ty = oty;
        const local = ctx.loc orelse self.bl.addLocal(sloc, self.abiCata(ty).size(), @intFromEnum(ty));
        ...
        .@"&" => {
            if (self.source.ast.exprs.getTyped(.Arry, e.oper)) |a| {
                if (a.ty.tag() == .Void) {
                    ...
                    const loc = ctx.loc orelse self.bl.addLocal(sloc, self.abiCata(ty).size(), @intFromEnum(ty));
...
pub fn emitBinOp(self: *Codegen, ctx: Ctx, expr: Ast.Id, e: *Expr(.BinOp)) EmitError!Value {
    ...
    switch (e.op) {
        ...
        .@"$||", .@"||" => |t| {
            ...
            if (lhs.ty.data() == .Union and e.rhs.tag() == .Tag and (e.op == .@"==" or e.op == .@"!=")) {
                const unon: tys.Union.Id = lhs.ty.data().Union;
                var tag_value = try self.emit(.{ .ty = unon.getTag(self.types) }, e.rhs);

                return self.checkTag(e.rhs, lhs, unon, &tag_value, e.op);
            }

            var rhs = try self.emit(.{ .ty = lhs.ty }, e.rhs);

            if (lhs.ty.data() == .Union and rhs.ty == lhs.ty.data().Union.getTag(self.types)) {
...
pub fn emitIndex(self: *Codegen, ctx: Ctx, expr: Ast.Id, e: *Expr(.Index)) EmitError!Value {
    ...
    if (e.subscript.tag() == .Range) {
        ...
        const loc = ctx.loc orelse self.bl.addLocal(sloc, self.abiCata(res_ty).size(), @intFromEnum(res_ty));
...
fn computeCaptures(self: *Codegen, ast_captures: Ast.Captures, scratch: *utils.Arena) ![]Types.Scope.Param {
    ...
    for (self.source.ast.exprs.view(ast_captures), captures) |cp, *slot| {
        ...
        slot.* = .{
            .id = .fromCapture(cp),
            .ty = val.ty,
            .value = if (cp.pos.flag.@"comptime") try self.partialEvalCapture(cp.id, &val) else 0,
        };
    }
...
pub fn emit(self: *Codegen, ctx: Ctx, expr: Ast.Id) EmitError!Value {
    ...
    return switch (ast.exprs.get(expr)) {
        ...
        .EnumWildcard => return self.report(expr, "enum wildcard can only be only specified as a union tag", .{}),
    };
}
...
pub fn emitSpill(self: *Codegen, expr: Ast.Id, value: *Value) void {
    value.* = .mkv(self.types.makePtr(value.ty), switch (value.id) {
        ...
        .Value => |v| self.bl.addSpill(self.src(expr), v, @intFromEnum(value.ty)),
        .Pointer => |p| p,
    });
}
...
pub fn emitDecl(
    ...
    errdefer |err| if (err != error.Unreachable) {
        self.scope.appendAssumeCapacity(.{
            .ty = .never,
            .name = ident.id,
            .@"comptime" = ident.pos.flag.@"comptime",
        });
        self.scope_pins.push(&self.bl, loc);
    };
    ...
    self.bl.resizeLocal(loc, self.abiCata(value.ty).size(), @intFromEnum(value.ty));
...
pub fn checkTag(
    ...
    std.debug.assert(unon.getTag(self.types) == tag_value.ty);

    const sloc = self.src(pos);

    return .mkv(.bool, self.bl.addBinOp(
        sloc,
        self.lexemeToBinOp(pos, op, tag_value.ty) catch unreachable,
        .i8,
        self.bl.addFieldLoad(
            sloc,
            union_value.id.Pointer,
            @intCast(unon.tagOffset(self.types)),
            self.abiCata(tag_value.ty).ByValue,
        ),
        tag_value.getValue(sloc, self),
    ));
}
    ...
    if (self.types.store.unwrap(expected.data(), .Nullable)) |v| if (v.inner == got.ty) {
        ...
        switch (abi) {
            .Imaginary => {
                ...
                const loc = self.bl.addLocal(self.src(expr), self.abiCata(expected).size(), @intFromEnum(expected));
...
pub fn unwrapTyConst(self: *Codegen, pos: anytype, cnst: *Value) !Types.Id {
    ...
    const value = try self.partialEvalConst(pos, cnst);
    return Types.Id.fromRaw(value, self.types) orelse {
...
pub fn resolveGlobal(
    ...
    if (path.len != 0) {
        ...
        var cur: Types.Id = @enumFromInt(@as(
            Types.IdRepr,
            @bitCast(global.data.slice(&self.types.ct)[0..@sizeOf(Types.Id)].*),
        ));
...
pub fn captureToValue(self: *Codegen, sloc: graph.Sloc, c: *const Types.Scope.Param) !Value {
    if (!c.id.has_value) {
        ...
        return .{ .ty = c.ty, .id = switch (self.abiCata(c.ty)) {
            ...
            .Imaginary => .Imaginary,
            .ByValue => .{ .Value = self.bl.addIntImm(sloc, .top, 0) },
            .ByValuePair, .ByRef => .{ .Pointer = self.bl.addIntImm(sloc, .top, 0) },
        } };
    }
...
pub fn loadIdent(self: *Codegen, expr: Ast.Pos, id: Ast.Ident) !Value {
    ...
    for (self.scope.items, 0..) |se, i| {
        if (se.name == id) {
            ...
            const value = self.scope_pins.getValue(i);
            if (value.kind == .GlobalAddr and self.target == .runtime) {
                self.types.queue(self.target, .init(.{
                    .Global = @enumFromInt(value.extra(.GlobalAddr).id),
                }));
            }
        ...
        if (binding.pos.flag.@"comptime") {
            ...
            captures[capture_idx] = .{
                .id = .fromCapture(.{
                    ...
                    .pos = .{ .index = 0, .flag = .{ .@"comptime" = true } },
                }),
                .ty = ty,
                .value = if (ty == .type)
                    ...
                    .Value => |v| try self.partialEvalCapture(expr, v),
                    .Expr => |ar| b: {
                        var val = try self.emitTyped(.{}, ty, ar);
                        break :b try self.partialEvalCapture(ar, &val);
                    },
                },
            };
...
fn pushReturn(
    ...
    if (self.abi.isByRefRet(ret_abi)) {
        call_args.arg_slots[0] = ctx.loc orelse
            self.bl.addLocal(self.src(pos), self.abiCata(ret).size(), @intFromEnum(ret));
        return 1;
    } else {
        return 0;
    }
    ...
    switch (abi) {
        ...
        .ByValue => {
            if (call_args.params[idx] == .Stack) {
                call_args.arg_slots[idx] = self.bl.addSpill(.none, value.getValue(sloc, self), @intFromEnum(value.ty));
            } else {
...
fn assembleReturn(
    ...
    return switch (ret_abi) {
        ...
        .ByValuePair => |pair| if (self.abi.isByRefRet(ret_abi)) b: {
            ...
            const slot = ctx.loc orelse self.bl.addLocal(sloc, self.abiCata(ret).size(), @intFromEnum(ret));
...
pub fn partialEvalLow(self: *Codegen, pos: u32, value: *Value) !Value {
    ...
    return switch (value.id) {
        .Imaginary => unreachable,
        .Value => .mkv(value.ty, res),
        .Pointer => .mkp(value.ty, res),
    };
}
...
pub fn partialEvalConst(self: *Codegen, pos: anytype, value: *Value) !i64 {
    var evaled = try self.partialEval(pos, value);
    return evaled.getValue(.none, self).extra(.CInt).value;
}

pub fn partialEvalCapture(self: *Codegen, pos: anytype, value: *Value) !i64 {
    const evaled = try self.partialEvalLow(self.posOf(pos).index, value);
    switch (evaled.id) {
        .Imaginary => unreachable,
        .Value => |val| return val.extra(.CInt).value,
        .Pointer => |ptr| switch (ptr.extra2()) {
            .GlobalAddr => |extra| return extra.id,
            else => unreachable,
        },
    }
}

pub fn partialEval(self: *Codegen, pos: anytype, value: *Value) !Value {
    return self.partialEvalLow(self.posOf(pos).index, value);
}
...
fn emitString(self: *Codegen, ctx: Ctx, data: []const u8, expr: Ast.Id) Value {
    ...
    const slice_loc = ctx.loc orelse
        self.bl.addLocal(self.src(expr), self.abiCata(slice_ty).size(), @intFromEnum(slice_ty));
...
fn emitDirective(
    ...
    switch (e.kind) {
        ...
        .bit_cast => {
            ...
            if (to_abi != .ByValue) {
                const loc = self.bl.addLocal(sloc, self.abiCata(ret).size(), @intFromEnum(ret));
        ...
        .field_name => {
            ...
            const idx = try self.partialEvalConst(args[1], &idx_value);
        ...
        .name_of => {
            ...
            const data = if (value.ty == .type) dt: {
                ...
                .Enum => |enum_ty| dt: {
                    ...
                    const id = try self.partialEvalConst(args[0], &value);
                    if (id >= fields.len)
        ...
        .simd => {
            ...
            const len: u64 = @bitCast(try self.partialEvalConst(args[1], &len_vl));
