...
const Mach = root.backend.Machine2;
...
pub const Ctx = struct {
    ...
    pub fn getReg(self: *Ctx, node: *FuncNode) Reg {
        return @enumFromInt(self.allocs[node.id]);
    }

    pub fn emitImmCmp(self: *Ctx, op_dt: graph.DataType, lhs: Reg, rhs: i32) void {
    ...
    pub fn prepareDivRegs(self: *Ctx, mnemonic: graph.BinOp, size: u64) void {
    ...
    pub fn stackOffset(self: *Ctx, reg: Reg, tag: RegTag) u64 {
        return @as(u64, (@intFromEnum(reg) - 16)) * tag.size() + self.slot_bases.get(tag);
    }

    pub fn ensureInstrSpace(self: *Ctx) void {
        self.opts.out.ensureUnusedCapacity(
            self.opts.gpa,
            max_instruction_size,
        ) catch unreachable;
    }

    pub fn stackBaseOf(self: *Ctx, instr: *Func.Node, inp: usize) i32 {
        return @intCast(switch (instr.inputs()[inp].?.extra2()) {
            .LocalAlloc => |n| n.getOffset(self.stack_layout),
            else => utils.panic("{f}", .{instr.inputs()[2].?}),
        });
    }

    pub fn emitBinopCmp(self: *Ctx, lhs: Reg, rhs: Reg, oper_dt: graph.DataType) void {
        // cmp lhs, rhs
    ...
    pub fn emitRexNoReg(self: *Ctx, ptr: Reg, idx: Reg, reg_size: u64) void {
    ...
    pub fn emitRexBinop(self: *Ctx, reg: Reg, ptr: Reg, idx: Reg, reg_size: u64) void {
    ...
    pub fn emitRex(self: *Ctx, reg: Reg, ptr: Reg, idx: Reg, reg_size: u64) void {
        const rex = Reg.rex(reg, ptr, idx, reg_size == 8);
    ...
    pub fn emitStackLea(self: *Ctx, dst: Reg, dis: i32) void {
        self.emitRex(dst, .rax, .rax, 8);
        self.emitByte(0x8d);
        self.emitIndirectAddr(dst, .rsp, .no_index, 1, dis);
    }

    pub fn emitRegOp(self: *Ctx, op: u8, dst: Reg, src: Reg) void {
        self.emitRex(dst, src, .rax, 8);
        self.emitBytes(&.{ op, Reg.Mod.direct.rm(dst, src) });
    }

    pub fn emitImmOp(self: *Ctx, op: u8, mod: u3, dst: Reg, imm: i64) void {
    ...
    pub fn emitSingleOp(self: *Ctx, op_base: u8, dst: Reg) void {
        const opcode = op_base + dst.scalarIndex();
        self.emitBytes(&.{ Reg.rex(.rax, dst, .rax, true), opcode });
    }

    pub fn emitByte(self: *Ctx, byte: u8) void {
        self.opts.out.appendAssumeCapacity(byte);
    }

    pub fn emitBytes(self: *Ctx, bytes: []const u8) void {
        self.opts.out.appendSliceAssumeCapacity(bytes);
    }

    pub fn emitImm(self: *Ctx, comptime T: type, int: i64) void {
        std.mem.writeInt(
            T,
            self.opts.out.addManyAsArrayAssumeCapacity(@sizeOf(T)),
            @truncate(int),
            .little,
        );
    }
...
pub fn emit(self: *X86_64Gen, func: *Func, opts: *Mach.Opts) void {
    errdefer unreachable;

    std.debug.assert(func.signature.call_conv == .systemv); // TODO: allow others

    func.iterPeeps(self, idealizeMach);
    func.gcm.buildCfg();
    const allocs = opts.regalloc.ralloc(X86_64Gen, func);
    ...
    ctx.stack_layout.arg_base += 8; // call adress

    prelude: {
        for (Reg.system_v.callee_saved) |r| {
            if (used_regs.contains(r)) {
                ctx.ensureInstrSpace();
                // push r
                ctx.emitSingleOp(0x50, r);
                ctx.stack_layout.arg_base += 8;
            }
        }

        opts.spec.stack_size = @intCast(ctx.stack_layout.arg_base);

        if (stack_size != 0) {
            // sub rsp, stack_size
            ctx.ensureInstrSpace();
            ctx.emitImmOp(0x81, 0b101, .rsp, stack_size);
        }
    ...
    for (postorder, 0..) |bb, i| ctx.schedules[bb.base.id] = @intCast(i);

    for (postorder, 0..) |bb, i| {
        ctx.block_offsets[i] = @intCast(ctx.code_size);

        emitBlockBody(&ctx, &bb.base);
        ...
        if (last.outputs().len == 0) {
            ...
            if (stack_size != 0) {
                // add rsp, stack_size
                ctx.ensureInstrSpace();
                ctx.emitImmOp(0x81, 0b000, .rsp, stack_size);
            }
            ...
            while (@as(?Reg, iter.next())) |r| {
                if (used_regs.contains(r)) {
                    // pop r
                    ctx.ensureInstrSpace();
                    ctx.emitSingleOp(0x58, r);
                }
            }

            // ret
            ctx.emitByte(0xc3);
        } else if (i + 1 == ctx.schedules[last.outputs()[@intFromBool(last.isSwapped())].get().id]) {
            // noop
            ...
            ctx.local_relocs.appendAssumeCapacity(.{
                .dest = ctx.schedules[last.outputs()[@intFromBool(last.isSwapped())].get().id],
                .offset = @intCast(ctx.code_size),
                .off = 1,
                .class = .rel32,
            });
            // jmp dest
            ctx.ensureInstrSpace();
            ctx.emitBytes(&.{ 0xE9, 0, 0, 0, 0 });
        }
    }

    for (ctx.local_relocs.items) |rl| {
        const size = @intFromEnum(rl.class);

        const dst_offset: i64 = ctx.block_offsets[rl.dest];
        const jump = dst_offset - rl.offset - size - rl.off; // welp we learned

        @memcpy(
            ctx.opts.out.items[rl.offset + rl.off ..][0..size],
            std.mem.asBytes(&jump)[0..size],
        );
    }
}

pub fn emitBlockBody(ctx: *Ctx, block: *FuncNode) void {
    errdefer unreachable;
    ...
    for (block.outputs()) |in| {
        ...
        if (instr.sloc != graph.Sloc.none) {
            ctx.opts.line_info.append(ctx.opts.gpa, .{
                .offset = @intCast(ctx.opts.out.items.len),
                .sloc = instr.sloc,
            }) catch unreachable;
        }
        ...
        switch (instr.extra2()) {
            ...
            .CInt => |extra| {
                if (extra.value == 0) {
                    // xor dst, dst
                    ctx.emitRegOp(0x31, ctx.getReg(instr), ctx.getReg(instr));
                } else {
                    // mov dst, $imm
                    ctx.emitSingleOp(0xb8, ctx.getReg(instr));
                    ctx.emitImm(i64, extra.value);
                }
            },
            .Poison => {},
            inline .F32, .F64 => |extra| {
                ctx.opts.big_constants.appendSlice(
                    ctx.opts.gpa,
                    std.mem.asBytes(&extra.imm),
                ) catch unreachable;

                const ty = @field(graph.DataType, @typeName(@TypeOf(extra.imm)));
                ctx.emitMemStoreOrLoad(ty, ctx.getReg(instr), .rip, null, .load);

                ctx.opts.addReloc(.{
                    .offset = 4,
                    .target = undefined,
                    .meta = .{ .addend = -4, .size = .@"4", .kind = .big_constant },
                });
            },
            .MemCpy => {
                // call id
                ctx.emitBytes(&.{ 0xe8, 0, 0, 0, 0 });
                ctx.opts.addReloc(.{
                    .offset = 4,
                    .target = ctx.opts.builtins.get(.memcpy),
                    .meta = .{ .addend = -4, .size = .@"4", .kind = .text },
                });
            },
            .MachSplit => {
                var lhs, var rhs = .{ ctx.getReg(instr), ctx.getReg(inps[0]) };
                if (lhs == rhs) continue;

                if (instr.data_type.isInt() and !rhs.isStack() and !lhs.isStack()) {
                    // mov dst, src
                    ctx.emitRegOp(0x89, rhs, lhs);
                } else if (instr.data_type.isFloat() and !rhs.isStack() and !lhs.isStack()) {
                    // movs[s/d] dst, src
                    ctx.emitByte(if (instr.data_type == .f32) 0xf3 else 0xf2);
                    ctx.emitRex(rhs, lhs, .rax, instr.data_type.size());
                    ctx.emitBytes(&.{ 0x0f, 0x11, Reg.Mod.direct.rm(rhs, lhs) });
                } else b: {
                    ...
                    if (lhs_stack and rhs.isStack()) {
                        // push [rsp+dis]
                        ctx.emitByte(0xff);
                        const lhs_off = ctx.stackOffset(lhs, tag);
                        ctx.emitIndirectAddr(@enumFromInt(0b110), .rsp, .no_index, 1, @intCast(lhs_off));

                        // pop [rsp+dis]
                        ctx.emitByte(0x8f);
                        const rhs_off = ctx.stackOffset(rhs, tag);
                        ctx.emitIndirectAddr(@enumFromInt(0b000), .rsp, .no_index, 1, @intCast(rhs_off));
                    ...
                    if (instr.data_type.isFloat()) {
                        ctx.emitByte(if (instr.data_type == .f32) 0xf3 else 0xf2);
                        ctx.emitRex(lhs, .rsp, .rax, instr.data_type.size());
                        ctx.emitBytes(&.{ 0x0f, if (lhs_stack) 0x11 else 0x10 });
                    } else {
                        ctx.emitRex(lhs, .rsp, .rax, instr.data_type.size());
                        ctx.emitByte(if (lhs_stack) 0x89 else 0x8b);
                    }

                    const offset = ctx.stackOffset(rhs, tag);
                    ctx.emitIndirectAddr(lhs, .rsp, .no_index, 1, @intCast(offset));
                }
            },
            .Phi => {},
            .GlobalAddr => |extra| {
                // lea dst, [rip+reloc]
                const dst = ctx.getReg(instr);
                ctx.emitRex(dst, .rax, .rax, 8);
                ctx.emitByte(0x8d);
                ctx.emitIndirectAddr(dst, .rip, .no_index, 1, null);

                ctx.opts.addReloc(.{
                    .offset = 4,
                    .target = @enumFromInt(extra.id),
                    .meta = .{ .addend = -4, .size = .@"4", .kind = .data },
                });
            },
            .FuncAddr => |extra| {
                // lea dst, [rip+reloc]
            ...
            .RepStosb => {
                // cld
                // rep stosb
                ctx.emitBytes(&.{ 0xfc, 0xf3, 0xaa });
            },
            .ConstGlobalStore => |extra| {
                // mov [rip+dis+offset], $vl
                const vl = extra.imm;
                const dis: i29 = @intCast(extra.dis);
                const imm_size: i16 = @intCast(@min(instr.data_type.size(), 4));
                ctx.emitMemConstStore(instr.data_type, vl, .rip, null);

                ctx.opts.addReloc(.{
                    .offset = @intCast(4 + imm_size),
                    .target = @enumFromInt(extra.id),
                    .meta = .{
                        .addend = dis - 4 - imm_size,
                        .size = .@"4",
                        .kind = .data,
                    },
                });
            ...
            .GlobalStore => |extra| {
                ...
                const src = ctx.getReg(inps[0]);
                ctx.emitMemStoreOrLoad(instr.data_type, src, .rip, null, .store);
                const dis: i29 = @intCast(extra.dis);
                ctx.opts.addReloc(.{
                    .offset = 4,
                    .target = @enumFromInt(extra.id),
                    .meta = .{ .addend = dis - 4, .size = .@"4", .kind = .data },
                });
            },
            .GlobalLoad => |extra| {
                // mov dst, [rip+dis+offset]
                const dst = ctx.getReg(instr);
                ctx.emitMemStoreOrLoad(instr.data_type, dst, .rip, null, .load);
                const dis: i29 = @intCast(extra.dis);
                ctx.opts.addReloc(.{
                    .offset = 4,
                    .target = @enumFromInt(extra.id),
            ...
            .Local => {
                // lea dst, [rsp+dis]
                const dst = ctx.getReg(instr);
                const dis = ctx.stackBaseOf(instr, 1);
                ctx.emitStackLea(dst, @intCast(dis));
            },
            .IndexLea => {
                // lea dst, [base+dis+index*scale]
                const dst = ctx.getReg(instr);
                const base = ctx.getReg(inps[0]);
                const index = ctx.getReg(inps[1]);
                const scale = instr.extra(.IndexLea).scale;
                const dis = instr.extra(.IndexLea).dis;

                ctx.emitRex(dst, base, index, 8);
                ctx.emitByte(0x8d);
                ctx.emitIndirectAddr(dst, base, index, @as(u64, 1) << @intCast(scale), dis);
            },
            .StackLea => {
                // lea dst, [rsp+dis]
                const dst = ctx.getReg(instr);
                const dis = instr.extra(.StackLea).dis + ctx.stackBaseOf(instr, 1);
                ctx.emitStackLea(dst, dis);
            },
            inline .OffsetLoad, .StackLoad => |extra, t| {
                // mov dst, [bse+dis]
                const dst = ctx.getReg(instr);
                const bse = if (t == .OffsetLoad) ctx.getReg(inps[0]) else .rsp;
                const dis: i32 = if (t == .OffsetLoad) extra.dis else (extra.dis + ctx.stackBaseOf(instr, 2));

                ctx.emitMemStoreOrLoad(instr.data_type, dst, bse, dis, .load);
            },
            inline .OffsetStore, .StackStore => |extra, t| {
                // mov [dst+dis], vl
                const dst = if (t == .OffsetStore) ctx.getReg(inps[0]) else .rsp;
                const vl = if (t == .OffsetStore) ctx.getReg(inps[1]) else ctx.getReg(inps[0]);
                const dis: i32 = if (t == .OffsetStore) extra.dis else (extra.dis + ctx.stackBaseOf(instr, 2));

                ctx.emitMemStoreOrLoad(instr.data_type, vl, dst, dis, .store);
            },
            inline .ConstStore, .ConstStackStore => |extra, t| {
                // mov [dst+dis], $vl
                const dst = if (t == .ConstStore) ctx.getReg(inps[0]) else .rsp;
                const vl = extra.imm;
                const dis: i32 = if (t == .ConstStore) extra.dis else (extra.dis + ctx.stackBaseOf(instr, 2));

                ctx.emitMemConstStore(instr.data_type, vl, dst, dis);
            },
            .Call => |extra| {
                if (extra.id == syscall) {
                    ctx.emitBytes(&.{ 0x0f, 0x05 });
                } else if (extra.id == graph.indirect_call) {
                    const ptr = ctx.getReg(inps[0]);
                    // call ptr
                    ctx.emitRexNoReg(ptr, .rax, 0);
                    ctx.emitBytes(&.{ 0xff, Reg.Mod.direct.rmSub(0b010, ptr) });
                } else {
                    // call id
                    ctx.emitBytes(&.{ 0xe8, 0, 0, 0, 0 });
            ...
            .Trap => {
                switch (instr.extra(.Trap).code) {
                    ...
                    => return,
                    // ud2
                    0 => ctx.emitBytes(&.{ 0x0f, 0x0b }),
                    else => unreachable,
                }
            },
            .If => {
                const cond = ctx.getReg(inps[0]);
                ...
                // test cond, cond
                ctx.emitRex(cond, cond, .rax, cond_size);
                ctx.emitBytes(&.{ opcode, Reg.Mod.direct.rm(cond, cond) });

                ctx.local_relocs.appendAssumeCapacity(.{
                    .dest = ctx.schedules[instr.outputs()[1].get().id],
                    .offset = @intCast(ctx.opts.out.items.len),
                    .class = .rel32,
                    .off = 2,
                });
                // jz dest
                ctx.emitBytes(&.{ 0x0f, 0x84, 0, 0, 0, 0 });
            },
            inline .CondJump, .ImmCondJump => |extra, t| {
                const lhs = ctx.getReg(inps[0]);
                const oper_dt = inps[0].data_type;

                if (t == .CondJump) {
                    const rhs = ctx.getReg(inps[1]);
                    ctx.emitBinopCmp(lhs, rhs, oper_dt);
                } else {
                    const rhs = extra.imm;
                    ctx.emitImmCmp(oper_dt, lhs, rhs);
                }
                ...
                // j[op] dst
                ctx.emitBytes(&.{ 0x0f, jmpCompOp(extra.op), 0, 0, 0, 0 });
            },
            inline .OpLoad, .OpStackLoad => |extra, t| {
                const op = extra.op;
                const op_dt = instr.data_type;
                const dst = ctx.getReg(instr);
                const lhs = if (t == .OpLoad) ctx.getReg(inps[1]) else ctx.getReg(inps[0]);
                const base = if (t == .OpLoad) ctx.getReg(inps[0]) else .rsp;
                const dis = if (t == .OpLoad) extra.dis else (extra.dis + ctx.stackBaseOf(instr, 2));

                switch (op) {
                    .iadd, .isub, .bor, .band, .bxor => {
                        std.debug.assert(lhs == dst);

                        if (op_dt == .i16) ctx.emitByte(0x66);
                        ctx.emitRexBinop(dst, base, .rax, op_dt.size());
                        ...
                        ctx.emitByte(opcode);
                        ctx.emitIndirectAddr(dst, base, .no_index, 1, dis);
                    },
                    else => utils.panic("{}", .{op}),
                }
            },
            inline .OpStore, .OpStackStore => |extra, t| {
                const op = extra.op;
                const base = if (t == .OpStore) ctx.getReg(inps[0]) else .rsp;
                const rhs = if (t == .OpStore) ctx.getReg(inps[1]) else ctx.getReg(inps[0]);
                const dis = if (t == .OpStore) extra.dis else (extra.dis + ctx.stackBaseOf(instr, 2));

                switch (op) {
                    .iadd, .isub, .bor, .band, .bxor => {
                        if (instr.data_type == .i16) ctx.emitByte(0x66);
                        ctx.emitRexBinop(rhs, base, .rax, instr.data_type.size());
                        const opcode: u8 = switch (op) {
                            .iadd => 0x01,
                            .band => 0x21,
                            .bor => 0x09,
                        ...
                        ctx.emitByte(opcode);
                        ctx.emitIndirectAddr(rhs, base, .no_index, 1, dis);
                    },
                    else => utils.panic("{}", .{op}),
                }
            },
            inline .ConstOpStore, .ConstOpStackStore => |extra, t| {
                const op = extra.op;
                const base = if (t == .ConstOpStore) ctx.getReg(inps[0]) else .rsp;
                const rhs = extra.imm;
                const crhs = std.math.cast(i8, rhs);
                const dis = if (t == .ConstOpStore) extra.dis else (extra.dis + ctx.stackBaseOf(instr, 2));

                switch (op) {
                    .iadd, .isub, .bor, .band, .bxor => {
                        if (instr.data_type == .i16) ctx.emitByte(0x66);
                        ctx.emitRexNoReg(base, .rax, instr.data_type.size());
                        ...
                        switch (instr.data_type) {
                            .i32, .i64, .i16 => ctx.emitByte(if (crhs == null) 0x81 else 0x83),
                            .i8 => ctx.emitByte(0x80),
                            else => utils.panic("{f}", .{instr.data_type}),
                        }

                        ctx.emitIndirectAddr(@enumFromInt(sub_opcode), base, .no_index, 1, dis);

                        if (crhs) |c| {
                            ctx.emitImm(i8, c);
                        } else {
                            switch (instr.data_type) {
                                .i8 => ctx.emitImm(i8, @intCast(rhs)),
                                .i16 => ctx.emitImm(i16, @intCast(rhs)),
                                .i32, .i64 => ctx.emitImm(i32, @intCast(rhs)),
                                else => unreachable,
                            }
                        }
                    },
            ...
            .ImmOp => |extra| {
                ...
                switch (op) {
                    ...
                    .ushr, .ishl, .sshr => {
                        ...
                        if (instr.data_type == .i16) ctx.emitByte(0x66);
                        ctx.emitRexNoReg(dst, .rax, instr.data_type.size());
                        ctx.emitBytes(&.{ opcode, Reg.Mod.direct.rmSub(sub_opcode, dst) });
                        ctx.emitImm(i8, @intCast(rhs));
                    },
                    .iadd, .isub, .bor, .band, .bxor => {
                        std.debug.assert(dst == lhs);
                        ...
                        if (op == .iadd and rhs == 1) {
                            // inc dst
                            ctx.emitBytes(&.{ inc_dec_op, Reg.Mod.direct.rmSub(0, dst) });
                        } else if (op == .isub and rhs == 1) {
                            // dec dst
                            ctx.emitBytes(&.{ inc_dec_op, Reg.Mod.direct.rmSub(1, dst) });
                        } else {
                    ...
                    .eq, .ne, .uge, .ule, .ugt, .ult, .sge, .sle, .sgt, .slt => {
                        // cmp lhs, $rhs
                        ctx.emitImmCmp(op_dt, lhs, rhs);

                        // setx dst
                        ctx.emitRexNoReg(dst, .rax, 1);
                        ctx.emitBytes(&.{
                            0x0F,
                            setCompOp(op),
                        ...
                        // movzx dst, al
                        ctx.emitRex(dst, dst, .rax, 8);
                        ctx.emitBytes(&.{ 0x0F, 0xB6, Reg.Mod.direct.rm(dst, dst) });
                    },
                }
            },
            .FusedMulAdd => {
                const dst = ctx.getReg(instr);
                const a = ctx.getReg(inps[0]);
                const b = ctx.getReg(inps[1]);
                const c = ctx.getReg(inps[2]);
                ...
                ctx.emitBytes(&Reg.vex3(a, b, c, ._0F38, ._66, wide, false));
                ctx.emitBytes(&.{ 0xB9, Reg.Mod.direct.rm(a, c) });
            },
            .BinOp => |extra| {
                const op = extra.op;
                ...
                switch (op) {
                    .ushr, .ishl, .sshr => {
                        ...
                        if (instr.data_type == .i16) ctx.emitByte(0x66);
                        ctx.emitRexBinop(dst, lhs, .rax, instr.data_type.size());
                        ...
                        ctx.emitBytes(&.{ opcode, Reg.Mod.direct.rmSub(sub_opcode, lhs) });
                    },
                    .fadd, .fsub, .fmul, .fdiv => {
                        std.debug.assert(lhs == dst);

                        switch (instr.data_type) {
                            .f32 => ctx.emitByte(0xF3),
                            .f64 => ctx.emitByte(0xF2),
                            else => unreachable,
                        }
                        ctx.emitRexBinop(dst, rhs, .rax, size);
                        ...
                        ctx.emitBytes(&.{ 0x0F, opcode, Reg.Mod.direct.rm(dst, rhs) });
                    },
                    .imul => {
                        std.debug.assert(lhs == dst);

                        ctx.emitRexBinop(dst, rhs, .rax, @max(size, 4));
                        ctx.emitBytes(&.{ 0x0F, 0xAF, Reg.Mod.direct.rm(dst, rhs) });
                    },
                    .iadd, .isub, .bor, .band, .bxor => {
                        std.debug.assert(lhs == dst);

                        ctx.emitRexBinop(rhs, dst, .rax, @max(size, 4));
                        ...
                        ctx.emitBytes(&.{ opcode, Reg.Mod.direct.rm(rhs, dst) });
                    },
                    .udiv, .sdiv, .smod, .umod => {
                        ...
                        // div rhs
                        ctx.prepareDivRegs(op, size);
                        if (instr.data_type == .i16) ctx.emitByte(0x66);
                        ctx.emitRexNoReg(rhs, .rax, size);
                        ...
                        ctx.emitBytes(&.{ opcode, Reg.Mod.direct.rmSub(sub_opcode, rhs) });

                        if (size == 1 and dest_reg == .rdx) {
                            // mov al, ah
                            ctx.emitBytes(&.{ 0x88, 0xe2 });
                        }
                    },
                    .fgt, .flt, .fge, .fle, .eq, .ne, .uge, .ule, .ugt, .ult, .sge, .sle, .sgt, .slt => {
                        ctx.emitBinopCmp(lhs, rhs, op_dt);
            ...
            .UnOp => |extra| {
                ...
                switch (op) {
                    ...
                    .sext => {
                        ...
                        ctx.emitBytes(&.{Reg.Mod.direct.rm(dst, src)});
                    },
                    .uext => {
                        if (src_size >= size) {
                    ...
                    .not => {
                        ...
                        if (instr.data_type == .i16) ctx.emitByte(0x66);
                        ctx.emitRexNoReg(dst, .rax, instr.data_type.size());
                        ctx.emitBytes(&.{ opcode, Reg.Mod.direct.rmSub(0b110, dst) });
                        ctx.emitImm(i8, 1);
                    },
                    .bnot, .ineg => {
                        // op dst
                    ...
                    .cast => {
                        ...
                        ctx.emitByte(0x66);
                        ctx.emitRex(dst, src, .rax, src_size);
                        ctx.emitBytes(&.{ 0x0f, opcode, Reg.Mod.direct.rm(dst, src) });
                    },
                    .fti => {
                        switch (src_dt) {
                            .f32 => ctx.emitByte(0xF3),
                        ...
                        ctx.emitRex(dst, src, .rax, size);
                        ctx.emitBytes(&.{ 0x0F, 0x2C, Reg.Mod.direct.rm(dst, src) });
                    },
                    .itf => {
                        switch (instr.data_type) {
                            .f32 => ctx.emitByte(0xF3),
                        ...
                        ctx.emitRex(dst, src, .rax, src_size);
                        ctx.emitBytes(&.{ 0x0F, 0x2A, Reg.Mod.direct.rm(dst, src) });
                    },
                    .fcst => {
                        switch (src_dt) {
                            .f32 => ctx.emitByte(0xF3),
                        ...
                        ctx.emitRex(dst, src, .rax, 0);
                        ctx.emitBytes(&.{ 0x0F, 0x5a, Reg.Mod.direct.rm(dst, src) });
                    },
                    .fneg => unreachable,
                }
            },
            .GetLane => |extra| get_lane: {
                ...
                ctx.emitRex(.rax, dst, src, instr.data_type.size());
                ctx.emitBytes(opcode);
                ctx.emitByte(Reg.Mod.direct.rm(dst, src));
                ctx.emitByte(extra.idx | extra.idx << 2 |
...
pub fn disasm(opts: Mach.DisasmOpts) void {
    errdefer unreachable;
...
pub fn run(env: Mach.RunEnv) Mach.RunError!usize {
