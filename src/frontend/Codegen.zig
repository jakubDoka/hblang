...
pub const Value = struct {
    ...
    pub fn normalized2(self: Value, pos: u32, gen: *Codegen) NormalizedNode2 {
        return switch (self.normalized(pos, gen)) {
            .empty => .empty,
            .value => |v| if (self.ty.isScalar(gen.types))
                .{ .value = v }
            else
                .{ .compound_value = v },
            .ptr => |p| .{ .ptr = p },
    ...
    pub fn normalized(self: Value, pos: u32, gen: *Codegen) NormalizedNode {
        return switch (self.node()) {
            ...
            .variable => |i| {
                ...
                const cata = gen.categoryOf(self.ty);
...
pub const NormalizedNode2 = union(enum) {
    empty,
    value: *BNode,
    compound_value: *BNode,
    ptr: *BNode,
};
...
pub fn emitUninit(self: *Codegen, pos: u32, ty: Types.Id) NormalizedNode {
    ...
    return switch (self.categoryOf(ty)) {
...
pub fn unitExpr(self: *Codegen, tok: Lexer.Token, ctx: Ctx, lex: *Lexer) UnitError!Value {
    return switch (tok.kind.expand()) {
        ...
        .idk => ik: {
            ...
            break :ik switch (self.categoryOf(ty)) {
        ...
        .@"{" => {
            ...
            while (iter.next()) {
                ...
                _ = self.typedExpr(.void, .{}, lex) catch |err| switch (err) {
        ...
        .match => {
            ...
            while (iter.next()) {
                ...
                {
                    var unreaches = false;
                    _ = self.typedExpr(.void, .{}, lex) catch |err| {
                        unreaches = err == error.Unreachable;
                    };

                    if (!unreaches) breaker.addBreak(&self.bl);
                }
            ...
            if (else_pos) |pos| {
                ...
                var unreaches = false;
                _ = self.typedExpr(.void, .{}, &elex) catch |err| {
                    unreaches = err == error.Unreachable;
                };
        ...
        .@"$if" => {
            if (self.peval(tok.pos, try self.expr(.{ .ty = .bool }, lex), bool) catch false) {
                const res = self.expr(.{ .ty = .void }, lex);
        ...
        .@"if" => {
            var cond = self.typedExpr(.bool, .{ .in_if_or_while = true }, lex) catch |err| switch (err) {
                error.Unreachable => return error.Unreachable,
        ...
        .@"$while", .@"$loop" => {
            ...
            while (fuel > 0 and (tok.kind == .@"$loop" or try self.peval(
                ...
                var unreached = false;
                _ = self.expr(.{ .ty = .void }, lex) catch |err| {
                    unreached = err == error.Unreachable;
                };
        ...
        .@"while" => {
            ...
            const cond = try self.expr(.{ .ty = .void }, lex);
        ...
        .@".(" => {
            ...
            const inner, const loc = self.emitLocHandleOpt(tok.pos, ty, ctx);
            var iter = lex.list(.@",", .@")");
            var i: usize = 0;

            switch (inner.data()) {
                .Builtin, .Pointer, .Slice, .Array, .FuncTy, .Enum, .Option => {
                    return self.report(tok.pos, "cant infer the type of the tuple, use <ty>.(..)", .{});
                },
                .Struct => |s| {
                    ...
                    if (i < layout.fields.len) {
                        return self.report(lex.cursor, "expected {} values, got {}", .{ layout.fields.len, i });
                    }
                },
            }
        ...
        .@"." => inferred_field: {
            ...
            if (lex.eatMatch(.@"(")) {
                ...
                const prev_errs = self.types.errored;

                const func, const args = try self.resolveFunction(
                    field.end,
                    ctx,
                    null,
                    item,
                ...
                break :inferred_field try self.emitCall(
                    ctx,
                    tok.pos,
                    func.get(self.types).sig(),
                    .{ .func = func },
                    args,
                    prev_errs,
                );
                ...
                return self.lookupIdent(scope.upcast(Types.AnyScope).pack(), field.view(lex.source)) orelse {
    ...
    switch (top.kind) {
        .@"." => {
            ...
            if (res.ty == .type) {
                ...
                const prev_error_count = self.types.errored;

                const llres = LLValue.init(tok.pos, res);
                const fnid, const args = try self.resolveFunction(
                    field.pos,
                    ctx,
                    llres,
                    value,
                ...
                switch (res.ty.data()) {
                    ...
                    .Struct => |s| {
                        ...
                        res = switch (res.normalized(field.pos, self)) {
                            ...
                            .value => |v| b: {
                                if (lfield.ty.size(self.types) == 0) {
                                    return .unit(lfield.ty);
                                }
        ...
        .@".?" => {
            ...
            res = switch (res.normalized2(top.pos, self)) {
                .empty => return self.report(top.pos, "unwrapping null, {} is uninhabited", .{ty}),
                .value => .unit(ty),
                .compound_value => |v| .value(ty, self.bl.addUnOp(
                    self.sloc(top.pos),
                    .ired,
                    self.types.abi.categorizeAssumeReg(ty, self.types),
                    v,
        ...
        .@".(" => {
            ...
            switch (inner.data()) {
                .Builtin, .Option, .FuncTy, .Pointer, .Array, .Slice, .Enum => return self.report(
                    top.pos,
        ...
        .@".[" => {
            ...
            const slot = ctx.loc orelse self.bl.addLocal(self.sloc(top.pos), 0, std.math.maxInt(u32));
        ...
        .@"(" => {
            ...
            const fid, const args =
                try self.resolveFunction(top.pos, ctx, null, res, lex, tmp.arena);
        ...
        .@":", .@":=" => {
            ...
            switch (value.normalized(tok.pos, self)) {
                .value, .empty => {},
                .ptr => |p| if (value.ty.isScalar(self.types)) {
                    value = .value(value.ty, value.load(eq.pos, self));
                } else {
                    const isUniqueLocal = opaque {
                        pub fn f(ptr: *BNode) bool {
        ...
        .@"||" => or_: {
            if (res.ty != .bool) {
                return self.report(top.pos, "TODO: || on non-bool", .{});
            }
            ...
            if ((top.kind == .@"!=" or top.kind == .@"==") and lex.eatMatch(.null)) {
                ...
                const opt = lhs.value.ty.data().Option.get(self.types);
                const layout = opt.getLayout(self.types);

                res = switch (lhs.value.normalized2(top.pos, self)) {
                    .empty => .value(.bool, self.bl.addIntImm(
                        self.sloc(top.pos),
                        .i8,
                        @intFromBool(tok.kind == .@"=="),
                    )),
                    .value => |v| .value(.bool, self.bl.addBinOp(
                        self.sloc(top.pos),
                        if (tok.kind == .@"==") .eq else .ne,
                        .i8,
                        v,
                        self.bl.addIntImm(self.sloc(top.pos), v.data_type, 0),
                    )),
                    .compound_value => |v| .value(.bool, self.bl.addBinOp(
                        self.sloc(top.pos),
                        if (tok.kind == .@"==") .eq else .ne,
                        .i8,
                        self.bl.addUnOp(
                            self.sloc(top.pos),
                            .ired,
                            self.categoryOf(opt.inner).Scalar,
                            self.bl.addBinOp(
                                self.sloc(top.pos),
                                .ushr,
                                v.data_type,
                    ...
                    .ptr => |p| .value(.bool, self.bl.addBinOp(
                        self.sloc(top.pos),
                        if (tok.kind == .@"==") .eq else .ne,
                        .i8,
                        ...
                        self.bl.addIntImm(self.sloc(top.pos), p.data_type, 0),
                    )),
                };
...
pub fn expect(self: *Codegen, lex: *Lexer, comptime to_expect: Lexer.Lexeme, comptime msg: []const u8) error{SyntaxError}!Lexer.Token {
    return lex.expect(to_expect) catch {
        if (0 == 1) std.debug.dumpCurrentStackTrace(@returnAddress());
...
pub fn resolveFunction(
    ...
    if (res.ty == .template) {
        ...
        errdefer self.deinitLLList(args.items);

        var i: usize = 0;
        if (caller) |cl| {
            const c = args.addOneAssumeCapacity();
            c.* = cl;
...
pub fn emitCall(
    ...
    defer self.deinitLLList(args);

    if (self.types.errored != prev_erred) {
...
pub fn instantiateTemplate(
    ...
    while (true) {
        ...
        const pos = lex.cursor;
        var value: LLValue = if (caller_vl) |*vl| b: {
            if (!is_any) self.normalizeCaller(vl, ty) catch {};
            break :b vl.*;
        } else .init(pos, self.expr(
        ...
        if (!is_any) self.typeCheckLL(.{}, &value, ty) catch {};
        ty = if (is_any) value.value.ty else ty;
        caller_vl = null;
        ...
        if (cmptime) {
            ...
            slot.ty = value.value.ty;

            const vl = try self.peval(pos, value.value, Types.ComptimeValue);
            value.deinit(self);
            ...
            if (is_any) {
                ...
                _ = try template_gen.pushVariable(
                    ident,
                    .ptr(ty, template_gen.bl.addStub(self.sloc(pos), .i64)),
                );
            }

            args.append(scratch.allocator(), value) catch unreachable;
        }
    ...
    if (!slot.found_existing) {
        ...
        std.debug.assert(std.mem.indexOfScalar(Types.Id, arg_tys, .any) == null);
        func.args = arg_tys;
...
pub fn lexemeToBinOpLow(self: Lexer.Lexeme, ty: Types.Id) !?graph.BinOp {
    const unsigned = ty.isBuiltin(.isUnsigned) or ty == .bool or ty == .type or ty.data() == .Pointer;
...
pub fn categoryOf(self: *Codegen, ty: Types.Id) Abi.Category {
    var buf = Abi.Buf{};
    return .fromSpec(self.types.abi.categorize(ty, self.types, &buf));
}
...
pub fn collectExports(types: *Types, gpa: std.mem.Allocator) !void {
    ...
    const func = switch (try self.@"fn"(&lex)) {
        ...
        .func => |v| v[0],
        .func_ty => return self.report(lex.cursor, "main function cannot be a fn type", .{}),
    };
...
pub fn typ(self: *Codegen, lex: *Lexer) error{Report}!Types.Id {
    return self.peval(
        lex.cursor,
        self.exprPrec(.{ .ty = .type }, lex, 1) catch |err| switch (err) {
            error.Unreachable => {
                return self.report(
                    lex.cursor,
...
pub fn runTest(name: []const u8, code: []const u8, gpa: std.mem.Allocator) !void {
    ...
    if (std.mem.indexOf(u8, name, "infinite") != null or @import("options").dont_simulate) {
        return;
    }
